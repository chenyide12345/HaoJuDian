...
+ (NSArray *)jsonArrayWithObjectArray:(NSArray *)objectArray {
    ...
    for (JDBaseModel *model in objectArray) {
        ...
        if ([self respondsToSelector:setter]) {
            ...
            if ([aDictValue isKindOfClass:[NSDictionary class]]) {
                ...
                if ([type isEqualToString:@"NSDictionary"] == NO && [type isEqualToString:@"NSMutableDictionary"] == NO) {
                    ...
                    if ([newClass isSubclassOfClass:[JDBaseModel class]]) {
                ...
                if (classString) {
                    for (id content in aDictValue) {
                        ...
                        //如果是该类的子类，说明能响应initWithDict:方法并且能建立映射字典。
                        if ([classInArray isSubclassOfClass:[JDBaseModel class]]) {
...
- (NSDictionary *)dictionaryRepresentation
{
    ...
    for (int i = 0; i < count; i++)
    {
        ...
        if (key && value)
        {
            if ([value isKindOfClass:[JDBaseModel class]])
            {
...
- (NSString *)jsonStringRepresentation
{
    ...
    if (error)
    {
        JDLog(@"Error occured when create json string representation, error: %@", error.localizedDescription);
    }
